...
协商缓存响应状态码304
                    ...
                    https本身没有漏洞，除非你非要访问非安全证书的网址

                    抓包工具能拿到https的明文数据 实际是因为 比如fiddler工具 需要先在客户端按照fiddler的根证书，然后就能正常充当中间人了
...
2   采取了多路复用，不会阻塞了 
    并且2采用了https 安全性也提高了 
    还有头部压缩的功能，如果同时发送多个请求，客户端和服务器共同维护一张表，相同字段只发送索引号，不发送真实数据 提高性能
    采用了二进制数据格式，不再是字符形式 大大降低了字节
    但是http2 如果底层tcp连接阻塞了，比如没有收到完整的数据，等待就阻塞了，所有的http请求响应就阻塞了 
    每个包都有序号，tcp收到4和6 就认为5丢了就要等待5到来才能把数据从缓冲区读上来 就阻塞了 也就是等待tcp的丢包重传了
3   因为2基于tcp的问题，3采用了底层是udp  基于udp的QUIC协议，但是目前并不普遍


                    由于tcp是基于四元组的 源/目的端口 源/目的 ip地址,手机从wifi变成4g ip地址就变化了 所以就需要重连了 
                    但是QUIC协议没有用四元组来表示连接，即使Ip地址变化也不卡顿，丝滑切换


                    重定向次数要减少，因为每次重定向浪费带宽，每次都要重新访问新服务器，然后万一中间涉及了代理服务器之类的 更麻烦
                    可以让重定向交给代理服务器去执行，客户端感知不到，客户端先发送请求到代理服务器，代理服务器再去重定向访问服务器

                    
                    如果要提升https的性能，就要升级硬件cpu，因为Https涉及密钥解密 是计算密集型 大量计算密钥
                    有的款式cpu支持aes-ni特性，这种指令级别优化了数据的加密解密传输过程，性能更快
                    使用指令 sort -u /proc/crypto | grep module | grep aes 可以查看linux系统cpu是否支持这个特性
                    msy@msy-virtual-machine:~/Network-Knowledge-Notes$ sort -u /proc/crypto | grep module | grep aes
                    module       : aesni_intel
                    msy@msy-virtual-machine:~/Network-Knowledge-Notes$ 
                    还有一种方式就是软件升级，升级linux内核从2到4 或者openssl1.0到1.1 但是风险大，生产环境怕出问题
                    还有一种就是协议优化，优化算法
